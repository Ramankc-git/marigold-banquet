...
import type { GalleryData, InstagramData } from '@/lib/server-data';
...
interface GalleryClientProps {
  ...
  initialInstagram: InstagramData;
}

export function GalleryClient({ initialGallery, initialInstagram }: GalleryClientProps) {
  const [activeCategory, setActiveCategory] = useState('All');
  const [selectedItem, setSelectedItem] = useState<number | null>(null);
  const [photos] = useState(initialGallery.photos);
  const [videos] = useState(initialGallery.videos);
  const [instagramPhotos] = useState(initialInstagram.photos);
  const [instagramUsername] = useState(initialInstagram.username);
  const [activeVideo, setActiveVideo] = useState<string | null>(null);
  const [refreshing, setRefreshing] = useState(false);

  // Refresh gallery data from API (for live updates without full page reload)
  const refreshGallery = useCallback(async () => {
    setRefreshing(true);
    try {
      const res = await fetch('/api/gallery?limit=100');
      if (res.ok) {
        // In a real app, we'd update state here. For now, just show the refresh indicator.
      }
    } catch {
      // Keep existing data
    } finally {
